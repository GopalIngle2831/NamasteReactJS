...
const Body = () => {
    ...
    //Creates a new component by adding Promoted label to RestaurantCard
    //RestaurantPromotedCard is the component returned by HOC (withPromotedLabel)
            ...
            <div className="restaurant-container flex flex-wrap justify-center gap-4">
                {filteredRestaurants.map((restaurant) => (
                    <Link to={"/restaurants/" + restaurant.id} key={restaurant.id}>
                        {restaurant.promoted ? <RestaurantPromotedCard resData={restaurant}/> : <RestaurantCard resData={restaurant} />}
                    </Link>
                ))}
            </div>
        </div>
